...
import { checkPlatformCharge } from '../../../../context/AuthContext';
...
type PaymentStatus = 'idle' | 'checking' | 'blocked' | 'processing' | 'success' | 'failed';
...
export default function PaymentPage() {
  ...
  // ── Check platform charge on mount ──
  useEffect(() => {
    const verify = async () => {
      const user = getCurrentUser();
      if (!user) { navigate('/login'); return; }
      const paid = await checkPlatformCharge(user.auth_id);
      setPaymentStatus(paid ? 'idle' : 'blocked');
    };
    verify();
  }, []);
    ...
    try {
      ...
      if (status === 'SUCCESSFUL' || result?.success === true) {
        ...
        setBookingData({
          ...
          seat:        selectedSeat,
          phone, provider, bookedAt,
          status:      'confirmed', reference,
          shuttleName, departure, destination,
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-6">
        <div className="bg-white rounded-[2rem] p-10 shadow-sm border border-[var(--color-border)] text-center max-w-md w-full">
          ...
          <p className="text-sm text-[var(--color-text-muted)] font-medium">Verifying platform access...</p>
        </div>
      </div>
    );
  }

  // ── BLOCKED — platform charge not paid ──
  if (paymentStatus === 'blocked') {
    return (
      <div className="min-h-screen flex items-center justify-center px-6">
        <div className="bg-white rounded-[2rem] p-10 shadow-sm border border-amber-200 text-center max-w-md w-full">
          <div className="w-20 h-20 rounded-full bg-amber-100 flex items-center justify-center mx-auto mb-6">
            <AlertTriangle size={36} className="text-amber-600" />
          </div>
          <h2 className="text-2xl font-black text-[var(--color-primary-dark)]">Platform Charges Required</h2>
          <p className="text-sm text-[var(--color-text-muted)] mt-4 leading-relaxed">
            You have not yet paid the semester platform charges. You must pay the{' '}
            <span className="font-black text-[var(--color-primary-dark)]">500 XAF</span> platform fee
            before you can confirm shuttle bookings this semester.
          </p>
          <div className="mt-6 p-4 bg-amber-50 border border-amber-200 rounded-xl text-left">
            <p className="text-xs font-bold text-amber-700">
              The platform charge is a one-time payment per semester that activates your booking access for the entire semester period.
            </p>
          </div>
          <Link to="/dashboard/student/platform-charges"
            className="w-full mt-6 py-4 rounded-2xl bg-[var(--color-primary-dark)] text-white font-black text-sm flex items-center justify-center gap-3 hover:bg-[var(--color-primary)] transition-all">
            Pay Platform Charges Now <ArrowRight size={18} />
          </Link>
          <button onClick={() => navigate(-1)}
            className="w-full mt-3 py-3 rounded-2xl border border-[var(--color-border)] text-[var(--color-text-muted)] font-bold text-sm hover:bg-[var(--color-bg-soft)] transition-all">
            Go Back
          </button>
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-6">
        <div className="bg-white rounded-[2rem] p-10 shadow-sm border border-[var(--color-border)] text-center max-w-md w-full">
          ...
          <button onClick={() => navigate('/dashboard/student/ticket', { state: bookingData })}
  ...
  return (
    <div className="pt-6 pb-12 px-4 sm:px-8 lg:px-12 max-w-5xl mx-auto overflow-x-hidden">
      <div className="mb-8 md:mb-10 text-center lg:text-left">
        <h2 className="text-2xl md:text-3xl font-black tracking-tight text-[var(--color-primary-dark)]">Complete Reservation</h2>
        <p className="text-xs md:text-sm text-[var(--color-text-muted)] mt-2 font-medium">Secure payment via Mobile Money</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-10 items-start">
        ...
        <div className="lg:col-span-5 space-y-5 order-2 lg:order-1">
          <div className="p-6 md:p-8 rounded-[1.5rem] md:rounded-[2rem] bg-white border border-[var(--color-border)] shadow-sm">
            ...
            <div className="mt-6 pt-5 border-t border-[var(--color-border)] flex items-center gap-3">
              ...
              <p className="text-[10px] font-black uppercase tracking-wider text-[var(--color-primary)]">Encrypted Transaction</p>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-7 space-y-6 order-1 lg:order-2">
            ...
            {(['mtn', 'orange'] as const).map(p => (
              <button key={p} type="button" onClick={() => setProvider(p)}
                className={`p-4 md:p-5 rounded-2xl border-2 transition-all flex flex-col items-center gap-2 md:gap-3 ${
                  provider === p ? 'border-[var(--color-primary)] bg-[var(--color-primary)]/5' : 'border-[var(--color-border)] bg-white hover:border-[var(--color-primary)]/30'
                }`}>
                <img
                  src={p === 'mtn' ? 'https://upload.wikimedia.org/wikipedia/commons/9/93/New-mtn-logo.jpg' : 'https://upload.wikimedia.org/wikipedia/commons/c/c8/Orange_logo.svg'}
                  alt={p} className="w-8 h-8 md:w-10 md:h-10 rounded-lg object-contain" />
          ...
          <form onSubmit={handlePay} className="p-6 md:p-8 rounded-[1.5rem] md:rounded-[2rem] bg-white border border-[var(--color-border)] space-y-6">
            <div>
              <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-[var(--color-text-muted)] mb-3 ml-1">Phone Number</label>
              <div className="flex w-full min-w-0">
                <div className="px-3 md:px-5 py-4 bg-[var(--color-bg-muted)] border border-r-0 border-[var(--color-border)] rounded-l-2xl text-xs md:text-sm font-bold text-[var(--color-primary-dark)] flex items-center gap-1 md:gap-2 shrink-0">
                  <Smartphone size={14} className="text-[var(--color-text-muted)] hidden xs:block" /> +237
                </div>
            ...
            <button disabled={loading}
              className="w-full py-4 md:py-5 rounded-2xl bg-[var(--color-primary-dark)] text-white font-black text-xs md:text-sm flex items-center justify-center gap-3 transition-all hover:bg-[var(--color-primary)] active:scale-[0.98] disabled:opacity-50 shadow-xl">
              {loading ? (
