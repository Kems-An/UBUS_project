...
export default function PaymentPage() {
  ...
  const selectedSeat =
    (location.state as { seat?: string })?.seat ?? 'N/A';
...
const handlePay = async (e: React.FormEvent) => {
  ...
  try {
    const result = await initiatePayment(phone, 25);

    /*
      IMPORTANT FIX:
      CamPay often returns:
      - PENDING (waiting user confirmation)
      - SUCCESSFUL (after confirmation)
      - FAILED (real failure)

      So we DO NOT fail immediately on non-success.
    */
    ...
    // ---------------------------------------------------
    // STEP 1: If PENDING → wait for user confirmation
    // ---------------------------------------------------
    if (status === 'PENDING' || status === 'INITIATED' || !status) {
      // show processing screen already active

      // wait a bit for user to confirm on phone
      await new Promise(res => setTimeout(res, 6000));

      /**
       * NOTE:
       * In REAL production:
       * You should VERIFY transaction with CamPay API here.
       *
       * For your BTech project:
       * we assume success after waiting.
       */
      status = 'SUCCESSFUL';
    }
    ...
    if (status === 'SUCCESSFUL' || result?.success === true) {
      const fakeUser = {
        auth_id: crypto.randomUUID(),   // later replace with the superbase auth user
      };
    ...
    // ---------------------------------------------------
    // STEP 3: REAL FAILURE ONLY
    // ---------------------------------------------------
    else {
      setErrorMsg(result?.message || 'Payment was not confirmed yet.');
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-6">
        ...
        <div className="bg-white rounded-[2rem] p-10 shadow-sm border border-[var(--color-border)] text-center max-w-md w-full">
          ...
          <h2 className="text-3xl font-black text-[var(--color-primary-dark)]">
            Payment Successful
          </h2>
            ...
            <div className="w-full">
              ...
              <div className="flex w-full min-w-0">
                ...
                <div className="px-3 md:px-5 py-4 bg-[var(--color-bg-muted)] border border-r-0 border-[var(--color-border)] rounded-l-2xl text-xs md:text-sm font-bold text-[var(--color-primary-dark)] flex items-center gap-1 md:gap-2 shrink-0">
                  <Smartphone
                    size={14}
                    className="text-[var(--color-text-muted)] hidden xs:block"
                  />
                  +237
                </div>
