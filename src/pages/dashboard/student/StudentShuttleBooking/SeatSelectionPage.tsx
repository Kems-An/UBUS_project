...
import { Bus, Info, Wifi, Zap, Accessibility, ArrowRight, RefreshCw, MapPin } from 'lucide-react';
...
export default function SeatSelectionPage() {
  ...
  const getSeatClasses = (id: string) => {
    const seat = getSeat(id);
    if (!seat) return '';
    ...
    if (seat.id === selected)
      return `${base} bg-[var(--color-primary-dark)] border-[var(--color-primary-dark)] text-white scale-110 shadow-lg`;
    if (seat.status === 'booked')
      return `${base} bg-[var(--color-bg-soft,#f1f5f9)] border-[var(--color-border)] text-[var(--color-text-muted)] cursor-not-allowed opacity-40`;
  ...
  const handleContinue = () => {
    ...
    navigate('/dashboard/student/payment', {
      state: { seat: selected, shuttleId, shuttleName, departure, destination },
    });
  };
    ...
    <div className="pt-6 pb-12 px-4 sm:px-8 lg:px-12 max-w-5xl mx-auto">
      ...
      <div className="bg-white border border-[var(--color-border)] rounded-[2.5rem] p-8 mb-10 shadow-sm">
        <div className="flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-5">
            ...
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--color-primary)] mb-1">Step 03</p>
              <h2 className="text-3xl font-black tracking-tight text-[var(--color-primary-dark)] leading-none">
                Select Your Seat
              </h2>
              <p className="text-xs text-[var(--color-text-muted)] font-medium mt-1">{shuttleName}</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7">
          <div className="bg-white rounded-[2rem] border border-[var(--color-border)] p-6 shadow-sm">
            ...
            <div className="flex items-center justify-center gap-2 mb-6 pb-4 border-b border-[var(--color-border)]">
              ...
              <span className="text-xs font-black uppercase tracking-widest text-[var(--color-text-muted)]">
                Front of Bus
              </span>
              <button onClick={() => loadSeats(true)} disabled={refreshing}
            ...
            <div className="flex justify-center mb-2">
              <div className="flex gap-2 text-[10px] font-black uppercase tracking-widest text-[var(--color-text-muted)]">
                <span className="w-10 sm:w-12 text-center">A</span>
                <span className="w-10 sm:w-12 text-center">B</span>
                <span className="w-6 text-center">|</span>
                <span className="w-10 sm:w-12 text-center">C</span>
                <span className="w-10 sm:w-12 text-center">D</span>
              </div>
            </div>
              ...
              {SEAT_LAYOUT.map((row, rowIdx) => (
                <div key={rowIdx} className="flex items-center justify-center gap-2">
                  {row.map((seatId, colIdx) =>
                    seatId === null ? (
                      <div key={colIdx} className="w-6 flex items-center justify-center">
                      ...
                      <div key={seatId}
                        ...
                        title={getSeat(seatId)?.status === 'booked' ? 'Already booked' : `Seat ${seatId}`}
                      >
                        {seatId}
                      </div>
                    )
            ...
            <div className="flex items-center justify-center gap-6 mt-6 pt-4 border-t border-[var(--color-border)]">
              {[
                { label: 'Available', cls: 'bg-white border-2 border-[var(--color-border)]' },
                { label: 'Selected',  cls: 'bg-[var(--color-primary-dark)] border-2 border-[var(--color-primary-dark)]' },
                { label: 'Booked',    cls: 'bg-[var(--color-bg-soft,#f1f5f9)] border-2 border-[var(--color-border)] opacity-40' },
              ].map(l => (
        ...
        <div className="lg:col-span-5 space-y-4">
          ...
          {/* Stats */}
          <div className="bg-white rounded-[2rem] border border-[var(--color-border)] p-6 shadow-sm">
            <h3 className="text-sm font-black uppercase tracking-widest text-[var(--color-text-muted)] mb-4">
              Shuttle Status
            </h3>
            <div className="space-y-3">
              {[
                { label: 'Available', value: available, color: 'text-emerald-600' },
                { label: 'Booked',    value: booked,    color: 'text-rose-500'    },
                { label: 'Total',     value: seats.length, color: 'text-[var(--color-primary-dark)]' },
              ].map(s => (
